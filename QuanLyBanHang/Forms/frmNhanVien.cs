...
namespace QuanLyBanHang.Forms
{
    public partial class frmNhanVien : Form
    {
            ...
            else
            {
                if (xuLyThem)
                {
                    ...
                    else
                    {
                        ...
                        nv.MatKhau = HashPassword(txtMatKhau.Text); // Mã hóa mật khẩu 
                ...
                else
                {
                    ...
                    if (nv != null)
                    {
                        ...
                        if (string.IsNullOrEmpty(txtMatKhau.Text))
                            ...
                            nv.MatKhau = HashPassword(txtMatKhau.Text); // Cập nhật mật khẩu mới
                        context.SaveChanges();
                    }
                }
        ...
        private void btnTimKiem_Click(object sender, EventArgs e)
        {
            ...
            var query = context.NhanVien.AsEnumerable();
            if (!string.IsNullOrEmpty(hoTen))
            {
                query = query.Where(nv => nv.HoVaTen.ToLower().Contains(hoTen));
            }

            if (!string.IsNullOrEmpty(dienThoai))
            {
                query = query.Where(nv => nv.DienThoai != null && nv.DienThoai.Contains(dienThoai));
            }

            if (!string.IsNullOrEmpty(diaChi))
            {
                query = query.Where(nv => nv.DiaChi != null && nv.DiaChi.ToLower().Contains(diaChi));
            }
            ...
            if (ketQua.Count == 0)
            {
                MessageBox.Show("Không tìm thấy nhân viên nào khớp với thông tin!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
                    ...
                    using (XLWorkbook workbook = new XLWorkbook(openFileDialog.FileName))
                    {
                        ...
                        if (table.Rows.Count > 0)
                        {
                            foreach (DataRow r in table.Rows)
                            {
                                ...
                                // Convert QuyenHan from possible string values to bool
                                string qStr = r["QuyenHan"].ToString();
                                bool qVal = false;
                                if (!bool.TryParse(qStr, out qVal))
                                {
                                    if (int.TryParse(qStr, out int qInt))
                                        qVal = qInt != 0;
                                    else
                                        qVal = qStr.Trim().Equals("1") || qStr.Trim().Equals("yes", StringComparison.OrdinalIgnoreCase) || qStr.Trim().Equals("y", StringComparison.OrdinalIgnoreCase);
                                }
                                nv.QuyenHan = qVal;
                                context.NhanVien.Add(nv);
                            }
